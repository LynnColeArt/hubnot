...
Details list uses existing catalog once, with rows {id,creator,state,opening_metadata,conflict,head_count}; state is complete canonical state or null. Opening metadata is copied from the verified opening event in each IssueView.History, located by view.ID; expose the full original map or {} for legacy, even when current state is null. Never interpret metadata namespaces. Total search across disjoint issue histories is linear with no added subprocesses. No head IDs nested in rows: existing heads command provides them. Same paging counters/ordering, details bound in cursor fingerprint. Default summaries byte-compatible in shape.
