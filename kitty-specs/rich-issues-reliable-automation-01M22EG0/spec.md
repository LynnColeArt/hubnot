...
| FR-005 | Guarded edit and resolution | Local edits require the exact observed head; ordinary resolution requires the complete observed head set; explicit staged resolution names current parents and validates its observed snapshot, preserving unconsumed heads. Stale requests append nothing. Distributed races remain explicit conflicts. | High | Open |
